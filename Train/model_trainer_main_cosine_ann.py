...
def main():
    ...
    if cloud_train:
        ...
        param_path = os.path.join(
            r"/home/perryman1997/saved_model_parameters", "test" + str(test_num)
        )
        batch_size = 384
    else:
        data_dir = r"C:\Users\Noel\Documents\THESIS\Data\artbench-10-imagefolder-split"
        param_path = os.path.join(
            r"C:\Users\Noel\Documents\THESIS\saved_" + str(test_num)
        )
        batch_size = 32
    ...
    optimizer = optim.SGD(
        params_to_update, lr=starting_lr, momentum=0.9, weight_decay=weight_decay
    )

    scheduler_ft = CosineAnnealingWarmRestarts(
        optimizer, T_0=50, T_mult=1, eta_min=1e-6, verbose=True
    )
